class UserInterface:
    ...
    # No Constructor as we do not need to accept any parameters
    # or set any class level variables. But, this does not
    # mean that we can't do that in the event that we do
    # need something.
    ...
    def display_menu_and_get_response(self):
        ...
        # While selecyion is not valid, re-get selection
        while not self._selection_is_valid(selection):
            # Print Error Message
            self._print_menu_error()
            # Re-print menu choice
            self._print_menu()
            self._print_prompt()
            # Get user input again
            selection = input()

        # At this point we know the choice is a valid number
        # of either a 1 or 2. So, can just return it.
        # No need for a Try, though we could add one.
    ...
    def _selection_is_valid(self, selection):
        """Verify selection is a valid choice"""
